...
for line in input_lines:
    ...
    prompt = f"<|im_start|>user\nMUST ANSWER AS INSTRUCTED.{line}<|im_end|> <|im_start|>assistant"
    res = generate_text(prompt)

    model_response = res[0]['generated_text'].split("<|im_start|>")[-1]

    data_dict[line] = model_response
  
    print(model_response)


df = pd.DataFrame(data_dict.items(), columns=['Question', 'Response'])
df.to_csv('/data/gpfs/projects/punim0619/Smith/data/outputs/MPT/MPT_MFT.csv')
